    ...
    return (
      <div className="connection-panel">
        ...
        <div className="connection-info">
          <div>Room: {relayClient.connectedUserId.slice(0, 8)}…</div>
          <div>Role: {relayClient.connectedRole}</div>
        </div>

        {createdRoom && (
          <div className="connection-room-info">
            <div>Room ID: <code>{createdRoom.roomId}</code></div>
            <div>GM Token: <code>{createdRoom.gmToken.slice(0, 8)}…</code></div>
            {createdRoom.accessCode && (
              <div>Access Code: <code>{createdRoom.accessCode}</code></div>
            )}
          </div>
        )}
      ...
      {mode === 'join' ? (
        <>
          ...
          <button
            ...
            disabled={isConnecting || !roomId || !token}
          >
